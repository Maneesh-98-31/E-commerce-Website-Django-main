...
class SSLPayment(LoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        ...
        mypayment.set_product_integration(total_amount=Decimal(f'{amount}'), currency='BDT',
                                          product_category='clothing', product_name='demo-product',
                                          num_of_item=no_of_items, shipping_method='YES', product_profile=userprofile)
    ...
    if request.method == "POST":
        ...
        while (1):
            ...
            try:
                ...
                with sr1.Microphone() as source2:
...
def seller_submit(request):
    if request.method=='POST':
        ...
        item_category=Category.objects.get(category=request.POST["item_category"])
        # drop = request.POST.get('select')
        #drop='artifacts'
        # d = request.POST['select']
        # drop=d[0:]
        x = Item(item_name = productname,item_category = item_category,item_image = image
                 , price = price,slug = productname.replace(' ', '_'),
                 description = description, seller_info = sellerinfo)
        x.save()
        return render(request,"seller.html")
...
#         some_var = request.POST.getlist('is_seller')
#         if len(some_var)==0:
#             x = User.objects.create_user(username=username1, password=password1, user_type=0)
#         else:
#             x = User.objects.create_user(username=username1, password=password1, user_type=1)
#
#         x.save()
#         return render(request, 'seller.html')
